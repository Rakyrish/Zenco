...
def ai_product_content(request):
    """
    Image-first product content generation for the admin publishing workspace.
    Uses the comprehensive content engine prompt (full authority-page sections).
    """
    from apps.products.content_engine import build_image_product_prompt, validate_product_content
    image_url = request.data.get('image_url')
    uploaded_image = request.FILES.get('image')
    prompt = request.data.get('prompt', '')
    content = [{'type': 'input_text', 'text': build_image_product_prompt(prompt)}]
    if image_url:
    ...
    try:
        ...
        try:
            issues = validate_product_content(json.loads(output_text))
        except Exception:
            issues = []
        return Response({'content': output_text, 'validation_issues': issues})
